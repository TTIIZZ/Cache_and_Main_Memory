...
int CheckNum(Address address, int size);
...
int main(void)
{
	...
	while (1)
	{
		...
		{
			// 동작 입력
			if (isatty(0)) printf("Input the access type [(R)ead / (W)rite / (P)rint / (Q)uit]: ");
			scanf(" %c", &type);
...
int Read()
{
	...
	if (CheckNum(address, size) == -1) return -1;
...
int Write()
{
	...
	if (isatty(0)) printf("What is the data to save?: ");
        scanf("%llx", &data);
